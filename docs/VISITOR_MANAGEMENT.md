...
- `visitor_name` - Optional full name
- `visitor_origin` - Where the visitor is from (Student, Faculty, Public, etc.)
- `check_in_time` - Timestamp of check-in (required)
...
- `visitor_room_id` - Foreign key to `visitor_rooms` (required)
...
- `comment` - Optional text comment (max 2000 chars)
- `survey_type` - Type of survey (general, service, facility, staff)
...
- Statistics shown:
  - Total visitors
  - Unique visitors (based on identifier)
...
System.change_visitor_log(visitor_log, attrs \\ %{})
```

#### Visitor Surveys

```elixir
# List visitor surveys with filters
System.list_visitor_surveys(opts \\ [])
# Options: node_id, location_id, from_date, to_date, rating, preload, limit

# Get a single visitor survey
System.get_visitor_survey!(id, opts \\ [])

# Create a visitor survey
System.create_visitor_survey(attrs)

# Update a visitor survey
...
# Get changeset for form
System.change_visitor_survey(visitor_survey, attrs \\ %{})
```
...
# Options: from_date, to_date, node_id, location_id
