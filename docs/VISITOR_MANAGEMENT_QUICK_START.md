...
- `visitor_logs` - Track visitor check-ins
- `visitor_surveys` - Store visitor feedback
...
- Locations should be assigned to the correct `node_id`
- Set `is_active = true` for locations that should appear in visitor check-in
- The `location_name` field will be shown to visitors
...
- **Check In:** http://localhost:4000/visitor
- **Submit Survey:** http://localhost:4000/visitor/survey

#### For Staff/Admin (Login Required)

- **View Statistics:** http://localhost:4000/manage/visitor/statistics
...
- **Virtual Keyboard:** Touch-friendly QWERTY keyboard for kiosks and tablets
- **Auto-reset:** Form automatically resets after check-in for the next visitor
...
- Uses existing `mst_locations` master data
- No separate visitor room management needed
- Manage locations through existing master data interface
- Set `is_active` to control visibility
- Locations displayed in alphabetical order
...
- Ensure locations exist in `mst_locations` for your node
- Check that locations have `is_active = true`
- Verify the locations are assigned to the correct `node_id`

**Q: Statistics showing zero visitors**
- Check your date range filter
- Verify you're looking at the correct node/location
- Ensure visitors have actually checked in
...
- Locations are managed through the existing master data interface
- Use your normal location management tools
- Contact your system administrator for access to master data management
