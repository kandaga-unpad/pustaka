export default {
  ...
  this.isOpen = false;
    ...
    this.calculate = () => {
      ...
      // Clamp into viewport
      const maxLeft = Math.max(margin, window.scrollX + window.innerWidth - panelWidth - margin);
  ...
  window.addEventListener('resize', this.calculate);
    ...
    this.outsideClick = (e) => {
      ...
      const anchor = this.el.querySelector('[data-panel-anchor]');
      if (!panel || !anchor) return;
      const clickedInside = panel.contains(e.target) || anchor.contains(e.target);
  ...
  document.removeEventListener('keydown', this.escapeHandler);
    ...
    if (this.panel && this.originalParent) {
      try {
        if (this.nextSibling) this.originalParent.insertBefore(this.panel, this.nextSibling);
