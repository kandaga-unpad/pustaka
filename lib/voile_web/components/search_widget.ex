defmodule VoileWeb.Components.SearchWidget do
  ...
  def handle_event("search_input", %{"value" => query}, socket) do
    ...
    if String.length(trimmed_query) >= 2 do
      ...
      {:noreply, assign(socket, :suggestions, [], :show_suggestions, false, :loading, false)}
    end
  end
  ...
  def handle_event("select_suggestion", %{"title" => title}, socket) do
    {:noreply,
     ...
     |> assign(:show_suggestions, false)
     |> push_redirect(to: "/search?q=#{URI.encode(title)}")}
  end
  ...
  end  defp type_badge_class("collection"), do: "bg-blue-100 text-blue-800"
