defmodule VoileWeb.Dashboard.Glam.Library.Circulation.Requisition.Show do
  ...
  def mount(_params, _session, socket) do
    ...
    unless Authorization.can?(socket, "circulation.view_transactions") do
      socket =
        socket
        |> put_flash(:error, "You don't have permission to access requisition details")
    ...
    case Circulation.approve_requisition(requisition) do
      {:ok, _requisition} ->
        {:noreply,
         ...
         |> push_navigate(to: ~p"/manage/glam/library/circulation/requisitions/#{requisition.id}")}
