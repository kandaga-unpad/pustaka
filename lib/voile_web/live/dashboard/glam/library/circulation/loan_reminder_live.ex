  ...
  def render(assigns) do
      ...
      <div class="bg-white shadow rounded-lg overflow-hidden">
        ...
        <%= if @members_with_loans == [] do %>
          <div class="p-8 text-center text-gray-500">
            <.icon name="hero-check-circle" class="w-16 h-16 mx-auto mb-4 text-gray-400" />
          ...
          <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                <tr :for={member <- @members_with_loans} class="hover:bg-gray-50">
                  <td class="px-6 py-4">
                    <div>
                      <div class="text-sm font-medium text-gray-900">
                        {member.member_name || "Unknown"}
                      </div>
                      <div class="text-sm text-gray-500">{member.member_email}</div>
                      <div class="text-xs text-gray-400">ID: {member.member_identifier}</div>
                    </div>
                  </td>
                  <td class="px-6 py-4">
              ...
              <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                <div>
                  <p class="text-sm text-gray-700">
                    {gettext("Page")} <span class="font-medium">{@page}</span>
                    {gettext("of")} <span class="font-medium">{@total_pages}</span>
                  </p>
                </div>
                <div>
    ...
    <!-- Member Loans Modal -->
      <%= if @selected_member_id do %>
        <div class="fixed inset-0 bg-gray-500/20 bg-opacity-75 transition-opacity z-50">
                      ...
                      <div class="flex justify-between items-start">
                        <div class="flex-1">
                          ...
                          <div class="mt-2 flex items-center text-sm">
                            <.icon name="hero-calendar" class="h-4 w-4 mr-1 text-gray-400" />
                            <span class="text-gray-600">
                              {gettext("Borrowed")}: {format_date(loan.transaction_date)}
                            </span>
                          </div>
                          <div class="mt-1 flex items-center text-sm">
                            <.icon name="hero-clock" class="h-4 w-4 mr-1 text-gray-400" />
                            <span class={[
                              "font-medium",
                              ...
                              {gettext("Due")}: {format_date(loan.due_date)}
                              {loan_status_text(loan)}
                            </span>
                          </div>
