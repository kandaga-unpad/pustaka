defmodule VoileWeb.Dashboard.Catalog.CollectionLive.FormComponent do
                ...
                <div class="space-y-4">
                  ...
                  <div class="mt-4">
                    ...
                    <label
                      ...
                      class="inline-flex items-center px-6 py-3 voile-gradient text-white font-medium rounded-lg shadow-lg transition-all duration-200 cursor-pointer transform hover:-translate-y-0.5"
                    >
            ...
            <%= for entry <- @uploads.thumbnail.entries do %>
              <div class="space-y-4">
                <div class="flex items-center space-x-3">
                  <div class="w-12 h-12 bg-voile-info/10 rounded-lg flex items-center justify-center">
                    <svg
  ...
  def handle_event("select_thumbnail_from_vault", %{"attachment_id" => attachment_id}, socket) do
    ...
    if vault_attachment do
      # Set the thumbnail path directly from the vault attachment
      thumbnail_path = Catalog.get_file_url(vault_attachment)

      # Update the form with the selected thumbnail
      updated_params = Map.put(socket.assigns.form.params, "thumbnail", thumbnail_path)
      changeset = Catalog.change_collection(socket.assigns.collection, updated_params)

      socket =
        socket
        |> assign(:form, to_form(changeset, action: :validate))
        |> put_flash(:info, "Thumbnail selected from asset vault successfully")
