  ...
  def render(assigns) do
    ...
    <div>
      <.header>
        {@title}
        <:subtitle>
          {gettext("Use this form to manage publishers records in your database.")}
        </:subtitle>
      </.header>
      ...
      >
        ...
        <.input field={@form[:contact]} type="text" label={gettext("Contact")} />
        <.button phx-disable-with={gettext("Saving...")}>{gettext("Save Publisher")}</.button>
      </.form>
    </div>
    """
  end
  ...
  defp save_item(socket, :edit, publisher_params) do
    case Master.update_publishers(socket.assigns.publisher, publisher_params) do
      {:ok, publisher} ->
        ...
        {:noreply,
         socket
         |> put_flash(:info, gettext("Publisher updated successfully."))
  ...
  defp save_item(socket, :new, publisher_params) do
    case Master.create_publishers(publisher_params) do
      {:ok, publisher} ->
        ...
        {:noreply,
         socket
         |> put_flash(:info, gettext("Publisher created successfully."))
         |> push_patch(to: socket.assigns.patch)}
