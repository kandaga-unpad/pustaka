  ...
  def render(assigns) do
    ...
    <div class="container mx-auto px-4 py-6">
      ...
      <div class="mb-6">
        ...
        <h1 class="text-3xl font-bold text-gray-900">Review Stock Opname</h1>
        
        <p class="text-gray-600 mt-1">{@session.title} - {@session.session_code}</p>
      </div>
       <%!-- Summary Cards --%>
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
          <div class="flex items-center justify-between mb-2">
            <p class="text-sm font-medium text-gray-600">Changes to Apply</p>
      ...
      <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
        <h2 class="text-lg font-semibold text-gray-900 mb-4">Session Scope</h2>
        
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          <div>
            <p class="font-medium text-gray-700">Nodes:</p>
            
            <p class="text-gray-600">{Enum.count(@session.node_ids)} node(s) included</p>
          </div>
          
          <div>
            <p class="font-medium text-gray-700">Collection Types:</p>
            
            <p class="text-gray-600">{Enum.join(@session.collection_types, ", ")}</p>
          </div>
          
          <div>
            <p class="font-medium text-gray-700">Scope Type:</p>
            
            <p class="text-gray-600">{@session.scope_type}</p>
          </div>
          
          <div :if={@session.scope_type == "collection" and @session.collection_id}>
            <p class="font-medium text-gray-700">Target Collection:</p>
            
            <p class="text-gray-600">{@session.collection.title}</p>
          </div>
          
          <div :if={@session.scope_type == "location" and @session.location_id}>
            <p class="font-medium text-gray-700">Target Location:</p>
            
            <p class="text-gray-600">{@session.location.name}</p>
          </div>
        </div>
      </div>
       <%!-- Librarian Work Summary --%>
      <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
        <h2 class="text-lg font-semibold text-gray-900 mb-4">Librarian Work Summary</h2>
       ...
       <%!-- Items with Changes --%>
      <div :if={@items_with_changes != []} class="bg-white rounded-lg shadow-sm p-6 mb-6">
        <h2 class="text-lg font-semibold text-gray-900 mb-4">
          Items with Changes ({Enum.count(@items_with_changes)})
        </h2>
        
        <div class="space-y-3">
          <div
            :for={item <- @items_with_changes}
          ...
          >
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
              <div :if={item.status_before != item.status_after} class="p-2 bg-white rounded">
                <p class="text-xs font-medium text-gray-700 mb-1">Status Change</p>
                
                <div class="flex items-center gap-2">
                  <span class="px-2 py-1 bg-gray-100 text-gray-700 rounded text-xs line-through">
                    {item.status_before}
                  </span> <.icon name="hero-arrow-right" class="w-4 h-4 text-gray-400" />
              ...
              <div :if={item.condition_before != item.condition_after} class="p-2 bg-white rounded">
                <p class="text-xs font-medium text-gray-700 mb-1">Condition Change</p>
              ...
              <div
                :if={item.availability_before != item.availability_after}
                class="p-2 bg-white rounded"
              >
                <p class="text-xs font-medium text-gray-700 mb-1">Availability Change</p>
                
                <div class="flex items-center gap-2">
                  <span class="px-2 py-1 bg-gray-100 text-gray-700 rounded text-xs line-through">
                    {item.availability_before}
                  </span> <.icon name="hero-arrow-right" class="w-4 h-4 text-gray-400" />
                  <span class="px-2 py-1 bg-green-100 text-green-700 rounded text-xs font-medium">
                    {item.availability_after}
                  </span>
                </div>
              </div>
              
              <div :if={item.location_before != item.location_after} class="p-2 bg-white rounded">
                <p class="text-xs font-medium text-gray-700 mb-1">Location Change</p>
                
                <div class="flex items-center gap-2">
                  <span class="px-2 py-1 bg-gray-100 text-gray-700 rounded text-xs line-through">
                    {item.location_before || "None"}
                  </span> <.icon name="hero-arrow-right" class="w-4 h-4 text-gray-400" />
                  <span class="px-2 py-1 bg-green-100 text-green-700 rounded text-xs font-medium">
                    {item.location_after || "None"}
                  </span>
                </div>
              </div>
            </div>
            
            <div :if={item.notes} class="mt-3 pt-3 border-t border-yellow-300">
              <p class="text-xs font-medium text-gray-700">Checker Notes:</p>
       ...
       <%!-- Missing Items --%>
      <div :if={@missing_items != []} class="bg-white rounded-lg shadow-sm p-6 mb-6">
        <h2 class="text-lg font-semibold text-gray-900 mb-4">
          Missing Items ({Enum.count(@missing_items)})
        </h2>
        
        <div class="space-y-2">
          <div
            :for={item <- @missing_items}
        ...
        <div class="space-y-4">
          ...
          <div class="border border-gray-200 rounded-lg p-4">
            <h3 class="font-medium text-gray-900 mb-2">
              <.icon name="hero-check-circle" class="w-5 h-5 inline text-green-600" /> Approve Session
            </h3>
          ...
          <div class="border border-gray-200 rounded-lg p-4">
            <h3 class="font-medium text-gray-900 mb-2">
              <.icon name="hero-arrow-uturn-left" class="w-5 h-5 inline text-yellow-600" />
              Request Revision
            </h3>
          ...
          <div class="border border-gray-200 rounded-lg p-4">
            <h3 class="font-medium text-gray-900 mb-2">
              <.icon name="hero-x-circle" class="w-5 h-5 inline text-red-600" /> Reject Session
            </h3>
    ...
    unless StockOpnameAuthorization.can_approve_session?(current_user, session) do
      ...
      unless session.status == "pending_review" do
        ...
        socket =
          ...
          |> assign(:items_with_changes, items_with_changes)
          |> assign(:missing_items, missing_items)
