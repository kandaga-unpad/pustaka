defmodule VoileWeb.SearchLive do
  ...
  def handle_event("search", %{"query" => query, "type" => search_type}, socket) do
    ...
    socket = assign(socket, :search_query, query, :search_type, search_type)

    if String.length(trimmed_query) >= 2 do
      send(self(), {:perform_search, trimmed_query, search_type, 1})
      {:noreply, assign(socket, :loading, true)}
    else
      {:noreply, assign(socket, :show_results, false, :results, %{})}
    end
  end
        ...
        <form phx-change="search" phx-submit="search" class="space-y-4">
          <div class="flex gap-2">
            ...
            >
              ...
              <option value="collections" selected={@search_type == "collections"}>Collections</option>
          ...
          <div class="bg-gray-50 rounded-lg p-4">
            <%= if @search_type == "universal" do %>
              <p class="text-gray-700">
                Found <strong><%= @results.total_results %></strong> results for "<strong><%= @search_query %></strong>"
                (<%= @results.collections.total %> collections, <%= @results.items.total %> items)
              </p>
            <% else %>
              <p class="text-gray-700">
                Found <strong><%= @results.total %></strong>
                <%= if @search_type == "collections", do: "collections", else: "items" %>
  ...
  defp search_results_section(assigns) do
    ...
    <div class="bg-white rounded-lg shadow-sm">
      ...
      <%= if @show_pagination and @pagination.total_pages > 1 do %>
        <.pagination pagination={@pagination} search_query={@search_query} type={@type} />
      <% end %>
    </div>
    """
  end
        ...
        <div class="flex-1">
          <h3 class="font-medium text-gray-900">
            <.link patch={~p"/collections/#{@collection.id}"} class="hover:text-blue-600">
              <%= Phoenix.HTML.raw(VoileWeb.SearchHTML.highlight_search_term(@collection.title, @search_query)) %>
            </.link>
          </h3>
          <%= if @collection.description do %>
            <p class="text-gray-600 mt-1 line-clamp-2">
              <%= Phoenix.HTML.raw(VoileWeb.SearchHTML.highlight_search_term(VoileWeb.SearchHTML.truncate_text(@collection.description), @search_query)) %>
            </p>
          <% end %>
        ...
        <div class="flex-1">
          <h3 class="font-medium text-gray-900">
            <.link patch={~p"/items/#{@item.id}"} class="hover:text-blue-600">
              <%= Phoenix.HTML.raw(VoileWeb.SearchHTML.highlight_search_term(@item.collection.title, @search_query)) %>
            </.link>
          </h3>
  ...
  defp get_user_role(socket) do
    ...
    cond do
      is_nil(current_user) -> "patron"
      current_user.user_role && current_user.user_role.name == "librarian" -> "librarian"
      current_user.user_role && current_user.user_role.name in ["admin", "superadmin"] -> "librarian"
      true -> "patron"
    end
  end
end
