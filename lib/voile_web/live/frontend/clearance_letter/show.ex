defmodule VoileWeb.Frontend.Clearance.ShowLetter do
  ...
  def render(assigns) do
    ...
    <%= if @clearance_disabled do %>
      <div class="min-h-screen flex items-center justify-center">
        <div class="text-center">
          <.icon name="hero-lock-closed" class="mx-auto h-16 w-16 text-gray-400" />
          <h1 class="mt-4 text-xl font-semibold text-gray-700">
            {gettext("Bebas Pustaka is not available yet")}
          </h1>
          <p class="mt-2 text-sm text-gray-500">
            {gettext("This feature will be introduced in a future release. Please check back later.")}
          </p>
        </div>
      </div>
    <% else %>
