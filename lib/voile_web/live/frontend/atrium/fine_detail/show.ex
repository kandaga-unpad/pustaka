    ...
    case Circulation.get_fine_with_details(fine_id) do
      {:ok, fine} ->
        ...
        if fine.member_id == member.id do
          {:ok, assign(socket, fine: fine, page_title: gettext("Fine Details"))}
        else
          {:ok,
           socket
           |> put_flash(:error, gettext("You don't have permission to view this fine."))
      ...
      {:error, _} ->
        {:ok,
         socket
         |> put_flash(:error, gettext("Fine not found."))
         |> push_navigate(to: ~p"/atrium")}
    end
  end
